...
const ShopData = () => {
  ...
  return (
    <div className='flex flex-col bg-gray-100 justify-center  '>
            <div className=' w-[100vw] h-[15%] flex items-center justify-center  overflow-x-auto m-12'>
               
                {
                    categories.map((category) => (
                        <div
                            ...
                            className={`w-fit min-w-fit h-8 mx-2 px-5 py-2 flex flex-col justify-center items-center text-sm border break-keep rounded-3xl cursor-pointer transition-all duration-300 ${(selectedCategories.includes(category))?'border-[--main-color] bg-[--main-color] text-white':' border-gray-500 bg-white text-gray-900'} `}>
                            {category.split("-").join(" ")}
                        </div>
                    ))
                }
